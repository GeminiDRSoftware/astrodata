...
def test_add_var_and_dq():
    ...
    with pytest.raises(
        AttributeError, match="SCI extensions should be appended with .append"
    ):
...
def test_round_trip_gwcs(tmp_path):
    ...
    spec_frame = cf.SpectralFrame(
        axes_order=(0,), unit=u.nm, axes_names="lambda", name="wavelength"
    )
    row_frame = cf.CoordinateFrame(
        1, "SPATIAL", axes_order=(1,), unit=u.pix, axes_names="y", name="row"
    )
    ...
    ad1[0].nddata.wcs = WCS(
        [(det_frame, distrans), (dref_frame, wavtrans), (rss_frame, None)]
    )
    ...
    np.testing.assert_allclose(wcs1(x, y), wcs2(x, y), rtol=1e-7, atol=0.0)

    y, w = np.mgrid[0:9:2, 500.025:500.12:0.0225]
    np.testing.assert_allclose(
        wcs1.invert(w, y), wcs2.invert(w, y), rtol=1e-7, atol=0.0
    )
