...
def test_update_header():
    ...
    assert list(hdr["COMMENT"]) == [
        "A super useful comment",
        "Another comment",
    ]
...
def test_section_basics():
    ...
    assert s.as_iraf_section() == "[1:1024,1:512]"
    assert s.as_slice() == (slice(0, 512), slice(0, 1024))
